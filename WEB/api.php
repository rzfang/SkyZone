...
class System
{
  ...
  public static $RZPswd = RZ_PSWD;
  public static $RZPswdSsnNm = SSN_RZ_PSWD;
...
class RZTool
{
  public static $Pswd = RZ_PSWD;
  public static $PswdSsnNm = 'RZPswd';
  public static $FdFlNm = 'rz_feed.json';

  /* Load Note.
    Return: JSON string | note string. */
  public static function NoteLoad()
  {
    global $Kw;
    ...
    if (!is_file($FP))
      return ReturnPack(-2, $Kw['RM']['NoSuchData'], '');

    $FS = file_get_contents($FP);

    return ReturnPack(0, $Kw['RM']['Done'], $FS);
  }

  /* Save Note.
    '$IA' = Info Array, often as '$_POST'.
      '$IA['NtStr']' = Note String.
    Return: JSON string | 0 as OK, < 0 as error. */
  public static function NoteSave($IA)
  {
    global $Kw;
    ...
    if (empty($IA['NtStr']) || !is_string($IA['NtStr']))
      return ReturnPack(-2, $Kw['RM']['StrangeValue'], -2);

    $FP = DAT_PTH . 'note.txt';
    $Str = $IA['NtStr'];
    $SL = file_put_contents($FP, $Str); // '$SL' = String Length.

    if (empty($SL))
      return ReturnPack(-3, $Kw['RM']['SystemError'], -3);
  ...
  {
    if (!is_file($FdFlPth) || filesize($FdFlPth) === 0)
      return array();

    $FdStr = file_get_contents($FdFlPth); // '$FdStr' = Feed String.
    $FdInfo = json_decode($FdStr, true); // 'FdInfo' = Feed Info.

    if (empty($FdInfo) || !is_array($FdInfo['Grp']) || !is_array($FdInfo['FdLst']))
      return array();
  ...
  /*
    '$IA' = Info Array, often link '$_POST'.
    Return: JSON string | array of feed data, or empty array as error. */
  public static function FeedList($IA)
  {
    global $Kw;
    ...
    $FdFlPth = DAT_PTH . self::$FdFlNm;
    $FdInfo = array('Grp' => array(), 'FdLst' => array());

    if (!is_file($FdFlPth) || filesize($FdFlPth) === 0)
    {
      $Rst = file_put_contents($FdFlPth, json_encode($FdInfo));

      if (empty($Rst))
        return ReturnPack(-2, $Kw['RM']['SystemError'], $FdInfo);

      return ReturnPack(0, $Kw['RM']['Done'], $FdInfo);
    }

    $FdStr = file_get_contents($FdFlPth);
    $FdInfo = json_decode($FdStr, true);

    if (empty($FdInfo) || !is_array($FdInfo['Grp']) || !is_array($FdInfo['FdLst']))
      return ReturnPack(-3, $Kw['RM']['SystemError'], array());
  ...
  /*
    '$IA' = Info Array, often link '$_POST'.
    Return: JSON string | id, or '' as error. */
  public static function FeedAdd($IA)
  {
    global $Kw;
    ...
    if (empty($IA) || empty($IA['URL']) || !IsURL($IA['URL']))
      return ReturnPack(-2, $Kw['RM']['StrangeValue'], '');

    $FdFlPth = DAT_PTH . self::$FdFlNm;
    $FdInfo = self::FeedInfoLoad($FdFlPth);
    $ID = time();

    if (empty($FdInfo))
      return ReturnPack(-3, $Kw['RM']['SystemError'], '');

    $FdInfo['FdLst'][] = array('ID' => $ID, 'URL' => $IA['URL'], 'GrpID' => '');
    $Rst = file_put_contents($FdFlPth, json_encode($FdInfo));

    if (empty($Rst))
      return ReturnPack(-4, $Kw['RM']['SystemError'], '');
  ...
  function FeedDel($IA)
  {
    ...
    if (empty($IA) || empty($IA['ID']) || !is_string($IA['ID']))
      return ReturnPack(-2, $Kw['RM']['StrangeValue'], -2);
    ...
    foreach($FdInfo['FdLst'] as $K0 => $V0)
    {
      if ($V0['ID'] == $IA['ID'])
       array_splice($FdInfo['FdLst'], $K0, 1);
    }
  ...
  function FeedroupChange($IA)
  {
    ...
    if (empty($IA) || empty($IA['ID']) || !is_string($IA['ID']) || !isset($IA['NewGrpID']))
      return ReturnPack(-2, $Kw['RM']['StrangeValue'], -2);
    ...
    foreach($FdInfo['FdLst'] as $K0 => $V0)
    {
      if ($V0['ID'] == $IA['ID'])
      {
        $FdInfo['FdLst'][$K0]['GrpID'] = !is_string($IA['NewGrpID']) ? '' : $IA['NewGrpID'];
  ...
  /*
    '$IA' = Info Array, often link '$_POST'.
      '$IA['GrpID']' = Group ID.
      '$IA['NewNm']' = New group Name.
    Return: JSON string | 0 as OK, < 0 as error. */
  public static function FeedGroupRename($IA)
  {
    global $Kw;
    ...
    if (!isset($IA['GrpID']) || !is_string($IA['GrpID']) || empty($IA['NewNm']) || !is_string($IA['NewNm']))
      return ReturnPack(-2, $Kw['RM']['StrangeValue'], -2);
    ...
    foreach($FdInfo['Grp'] as $K0 => $V0)
    {
      if ($V0['ID'] == $IA['GrpID'])
      {
        $FdInfo['Grp'][$K0]['Nm'] = $IA['NewNm'];
        $Rst = file_put_contents($FdFlPth, json_encode($FdInfo));
    ...
    return ReturnPack(-5, $Kw['RM']['NoSuchData'], -5);
  }

  /*
    '$IA' = Info Array, often link '$_POST'.
      '$IA['Nm']' = group Name.
    Return: JSON string | array of feed group data, or empty array as error. */
  public static function FeedGroupAdd($IA)
  {
    global $Kw;
    ...
    if (empty($IA['Nm']) || !is_string($IA['Nm']))
      return ReturnPack(-2, $Kw['RM']['StrangeValue'], array());
    ...
    $NewGrp = array('Nm' => $IA['Nm'], 'ID' => time());
    $FdInfo['Grp'][] = $NewGrp;
    $Rst = file_put_contents($FdFlPth, json_encode($FdInfo));

    if (empty($Rst))
      return ReturnPack(-4, $Kw['RM']['SystemError'], array());

    return ReturnPack(0, $Kw['RM']['Done'], $NewGrp);
  }
  ...
  /*
    '$IA' = Info Array, often link '$_POST'.
      '$IA['ID']' = group ID.
    Return: JSON string | 0 as OK, < 0 as error. */
  public static function FeedGroupDel($IA)
  {
    global $Kw;
    ...
    // check feeds to unlink with target group.
    foreach($FdInfo['FdLst'] as $K0 => $V0)
    {
      if ($V0['GrpID'] == $IA['ID'])
        $FdInfo['FdLst'][$K0]['GrpID'] = '';
    }
  ...
  public static function UploadCodeMapLoad($IA)
  {
    global $Kw;
    ...
    $UplFlA = array_slice(scandir(DAT_PTH . 'upload/', 0), 2);

    if (count($UplFlA) < 1)
      return ReturnPack(0, $Kw['RM']['NoSuchData'], array('CdMp' => '', 'FlA' => array()));

    if (!in_array('codemap.txt', $UplFlA))
      return ReturnPack(-2, $Kw['RM']['SystemError'], array('CdMp' => '', 'FlA' => $UplFlA));

    return ReturnPack(0, $Kw['RM']['Done'],
                      array('CdMp' => file_get_contents(DAT_PTH . 'upload/codemap.txt'), 'FlA' => $UplFlA));
  }

  public static function UploadCodeMapSave($IA)
  {
    global $Kw;
    ...
    if (empty($IA['CdMp']) || !is_string($IA['CdMp']))
      return ReturnPack(-2, $Kw['RM']['StrangeValue']);

    $CdMpFlPth = DAT_PTH . 'upload/codemap.txt';
    $Rst = file_put_contents($CdMpFlPth, $IA['CdMp']);
